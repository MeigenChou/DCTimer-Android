...
public class FullCube {
    ...
    public static FullCube randomCube(int shape) {
        return randomCube(shape, r.nextInt(40320), r.nextInt(40320));
    }

    public static FullCube randomCube(int shape, int corner, int edge) {
        shape = Shape.ShapeIdx[shape];
        FullCube f = new FullCube();
        int[] pc = new int[8], pe = new int[8];
        Utils.set8Perm(pc, 8, corner);
        Utils.set8Perm(pe, 8, edge);
        int cc = 0, ec = 0;
        for (int i = 0; i < 24; i++) {
            if (((shape >> i) & 1) == 0) {	//edge
...
//		byte[] prm = new byte[8];
        ...
        //convert to number
        sq.cornperm = Utils.get8Perm(prm, 8);
...
//		else { a=12; sq.botEdgeFirst=true;  }
        ...
        sq.edgeperm = Utils.get8Perm(prm, 8);
