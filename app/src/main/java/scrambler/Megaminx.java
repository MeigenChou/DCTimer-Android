...
public class Megaminx {
    ...
    public int[] state = new int[11 * 12];
    private int[] seq;	// move sequences
    ...
    private int[] applyMove(int[] state, short[] movePerm) {
        int[] stateNew = new int[11 * 12];
    ...
    private void scramble() {
        for (int i = 0; i < linenbr * linelen; i++) {
            seq[i] = (int) (Math.random() * 2);
        }
    }

    public String scramblestring(int scrLen) {
        linenbr = (byte) Math.ceil(scrLen / 10.0);
        seq = new int[linelen * linenbr];
        scramble();
