...
public class Cube {
	...
	public static void appendmoves(char[] sq, char[] axsl, int tl, int la){
		for(int sl=0; sl<tl; sl++){  // for each move type
			if(axsl[sl]!=0){       // if it occurs
				...
				// store move
				sq[seql++]=(char) ((m*6+sa)*4+q);
			}
		}
	}
	public static void parse(int siz){
		size = (byte) siz;
	}
	private static void scramble() {
		...
		if(mult || (size&1)!=0) tl--;
		//set up bookkeeping
		char[] axsl=new char[tl];    // movement of each slice/movetype on this axis
		char[] axam={0,0,0}; // number of slices moved each amount
		int la = -1; // last axis moved
		// initialise this scramble
		seq=new char[seqlen]; // moves generated so far
		seql=0;
		...
		while(seql + moved <seqlen ){
			...
			moved++;
			axsl[sl]=(char) (q+1);// mark the slice has moved amount
		}
