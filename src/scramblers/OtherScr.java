...
public class OtherScr {
		...
		for(j=0; j<len; j++) {
			...
			do{
				...
				if(first != lastaxis || donemoves[second] == 0) {
					if(first == lastaxis) {
						donemoves[second] = 1;
						scr.append(turns[first][second] + rndEl(suffixes) + " ");
					} else {
	...
	public static String megascramble(String[][] turns, String[] suffixes) {
		return megascramble(turns, suffixes, Mi.scrLen);
	}
		...
		for(j=0; j<Mi.scrLen; j++) {
			...
			do{
				...
				if (donemoves[second] == 0) {
					donemoves[second] = 1;
					scr.append(rndEl(turns[first][second]) + rndEl(suffixes) + " ");
					done = 1;
				}
			} while(done == 0);
		}
...
//	}
	...
	public static byte[] imagestr(String scr, int size) {
		switch (size) {
		case 3:
