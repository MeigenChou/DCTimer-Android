...
public class Util {
	...
	public static void init(Handler handler) {
		...
		try {
			...
			Center2.initRl();
			min2phase.Tools.read(Center2.ctrot, in);
			min2phase.Tools.read(Center2.ctmv, in);
			in.read(Center2.ctprun);
			handler.sendEmptyMessage(22);
			min2phase.Tools.read(Center3.ctmove, in);
			Center3.createPrun();
			in.close();
		} catch (Exception e) {
			Center1.initSym();
			...
			handler.sendEmptyMessage(21);
			Center2.initRl();
			Center2.initCt(handler);
			handler.sendEmptyMessage(22);
			Center3.createMove(handler);
			Center3.createPrun();
			try {
				OutputStream out = new BufferedOutputStream(new FileOutputStream("sdcard/center.dat"));
				write(Center1.sym2raw, 1113, out, null);
				write(Center1.ctsmv, 15582, 36, out);
				min2phase.Tools.write(Center2.ctrot, out);
				min2phase.Tools.write(Center2.ctmv, out);
				out.write(Center2.ctprun);
				min2phase.Tools.write(Center3.ctmove, out);
				out.close();
			} catch (Exception e2) { }
		}
		...
		try {
			...
			try {
				...
				write(Edge3.eprun, 1538, out, handler);
				out.close();
			} catch (Exception e2) { }
		}
		ini = true;
	...
	static void read(int[] arr, int r, InputStream in) throws IOException {
		int len = arr.length / r;
		byte[] buf = new byte[r * 4];
		int start = 0;
		for (prog=0; prog<len; prog++) {
			in.read(buf);
			for (int i=0; i<r; i++) {
	...
	static void write(int[] arr, int s, OutputStream out, Handler handler) throws Exception {
		...
		int start = 0;
		for (prog=0; prog<len; prog++) {
			if(handler != null && prog % (387576/s) == 0)
